...
> **➡️ PRÓXIMA INSTÂNCIA (ABERTAS locais, prontas):** (a) **OP-1b** — materializar `engines/tdc/art128.py` = `MAX(VTcd vigente ; VTcd Declaração × IPCA §2º)` + eval (spec no banner GARIMPO M6 abaixo + `docs/TESE-VTCD-MAXIMO-ART128.md`); (b) **OP-1c** — conferir `fp.py` produz R$ 2.352,06 para cedente-parque (Art. 127). **Bloqueadas por dado/infra do dono:** T8/G1/G3/G4 (LOTES shapefiles · IPTU_2026 no Storage) · B-20/Fase-B (PII pesado Supabase) · B-8/B-9 (lane do Drive) · **M6** (auditoria profunda do Drive — só DEPOIS das pendências atuais, decisão do dono).
>
