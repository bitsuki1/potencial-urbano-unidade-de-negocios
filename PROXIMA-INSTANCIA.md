...
> **✅ INGESTÃO INICIADA (MOU "pode" 2026-07-13) — 3 normas TDC no corpus citável (38→41 leis; 80 no RAG):**
> ① **Lei 18.298/2025** (gap) — revisão do PIU Arco Pinheiros/ZOE-Butantan; **vigência (1.6): Arts. 3-5 com eficácia SUSPENSA pela ADI 2007332-76.2026.8.26.0000**, Arts. 1,2,6,7 em vigor. ② **Decreto 63.999/2024** — atualiza o **Quadro 14** (+4,5%, art. 118 PDE) e fixa o **valor de ref. do incentivo de TDC p/ parques (art. 127) em R$ 2.194,50/m²** → **fecha o loop de auditoria do engine** (art128/oodc já usam esses números; 1.3). ③ **Decreto 58.176/2018** — altera o § 1 art. 20 do **Decreto 57.536/2016** (TDC sem doação, já citado). Todos verbatim do **portal oficial** (não da cópia truncada do Drive), fatiados+indexados, gates verdes.
> **FERRAMENTA NOVA:** `scripts/baixar_norma_portal.py` (captura verbatim do portal, anti-truncamento) + `scripts/triar_fila_portal.py` (triagem por conteúdo). **RECEITA (obrigatória p/ cada nova lei):** baixar→autorar `.json`→stub `.md`→`promover_entrada.py`→**registrar domínio em `carimbar_dominio.py` (senão o gate `consolidar` quebra)**→`fatiar`+`indexar`.
> **➡️ FILA TDC RESTANTE (11, triadas por conteúdo em `TRIAGEM-FILA-PORTAL.csv`):** decretos 58.955/19, 57.565/16, 59.164/19, 59.886/20, 58.028/17, 63.437/24, 64.018/25, 63.728/24 · leis 17.217/19, 17.734/22, 18.209/24. Slugs a rever (portal deu sem_texto): dec 57.776/57.299/57.521/57.770 (família 57.5xx TDC). **IPTU (9) = trilha DIFERIDA** (base inicial = TDC). Re-extrair os 49 HOLD ILEGÍVEL. Continuar sob o "pode" já dado.
