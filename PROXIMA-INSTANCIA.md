...
> **★★★★★★★★★★★★★★★★★★★★★★ PU 18 (2026-07-11, GARIMPO M6) — CAÇA A OPORTUNIDADE COMEÇOU · ACHADO CARRO-CHEFE (VTcd) · SWARM DE CONTAGEM CONTIDO.**
> Missão do dono: _"existem inúmeras oportunidades de melhores condições no TDC/IPTU nos documentos que você dispensou. Vamos começar."_ Feito nesta passada:
> **① CARRO-CHEFE — VTcd (`docs/OPORTUNIDADES-M6-TDC-IPTU.md`):** o preço em produção usa o Quadro 14 **ano-ref 2025** (`enriquecer_oficial.py:178` ← `q14_cedentes_2025.csv`), **uma safra atrás**. **OP-1a (LIMPA, +7,18%):** falta aplicar o exercício **2026** (Dec. 64.884/2025 + Portaria SMUL 8/2026) → sobe o preço legal de TODO cedente; NÃO apliquei (muda número ao cliente — quer o arquivo exato `Atualizacao_Q14_anoref2026.csv` + seu aval). **OP-1b (TESE, maior):** Art. 128 §2º corrige VTcd por IPCA (fator jan/2014→jun/2026 = **1,9330 = +93,3%**, série já em repo) mas a prospecção não aplica ("§2º N/A até Declaração protocolada"). Tese = VTcd **máximo rastreável** entre (Quadro 14 vigente) e (anexo 2014 + IPCA §2º), por SQL. Camada Advogado.
> **② OUTRAS OPORTUNIDADES registradas:** OP-2 (26 cedentes falso-positivo na quadra 013.036 Praça Benedito Calixto — CONPRESP Res. 01/2025; **26/4360** confirmados por grep, falta bater no PDF id `12UzO...`); OP-3 FUNDURB latência; OP-4 testadas de papel (CTLU); OP-5 Arco Pinheiros/AIU-ACP (Lei 18.222/2024); OP-6 CADIN/CND gate fiscal. Want-list em `docs/INVENTARIO-E-LACUNAS-IPTU-TDC.md`.
> **③ CONTAGEM DO DRIVE (passo 2 do dono, #22):** ~3.000-3.500 arquivos ÚNICOS, mas o número cru engana — dominado por (a) **lixeira/duplicatas** (`99_LIXEIRA_DUPLICADOS` ~2.344 + QUARENTENA + PARA_DELETAR), (b) **fragmentos de shapefile GIS** (~4.340 registros .shx/.dbf/.shp — cada mapa = 5-8 arquivos → poucas centenas de camadas reais), (c) **markdown gerado por nós** (~3.325 .md, staging). O **corpus mineável** = PDFs (poucas centenas únicos após dedup) + planilhas. **Swarm de contagem virou runaway** (agentes a 100-490k tokens, dupla-contagem) — **PAREI todos**; #22 fica FECHADO com o shape (precisão de inteiro não é o entregável).
> **Depende do dono (garimpo):** ① aval p/ refletir +7,18% no preço (ou traz o arquivo 2026); ② abrir tese §2º; ③ aval p/ conferir+sinalizar os 26 (remover = sua palavra). **Gate segue 32/32** (só docs nesta passada).
>
