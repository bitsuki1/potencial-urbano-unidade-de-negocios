...
| **D-DONO-5** | **"Lançar diversas lentes para ler tudo que temos"** — estudo completo do inventário antes de subir. | Workflow `ler-inventario-dados` (4 lentes) → `CATALOGO-MESTRE-DADOS-2026-07-01.md`. **FEITO.** Achados: socios/empresas são adquiridas; "outra IPTU"=`iptu-2020-cep01`; Fp/Quadro 6 EXISTE. | — (resolvido) |
| **D-DONO-6** | **`holdings.csv` = ADQUIRIDO** (sobe) + **ORDEM: enriquecer PRIMEIRO com as OFICIAIS, depois com as ADQUIRIDAS.** | 2 fases: **Fase A (oficiais)** monta o produto sobre base confiável/rastreável (SQL, Atc, V, zona, potencial, dono-parcial-oficial); **Fase B (adquiridas)** — socios/empresas/holdings/iptu-2020 — só ENRIQUECE por cima (dono/beneficiário em mais casos), marcado como camada adquirida. | Pauta 2 (método de subida) |
