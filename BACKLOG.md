...
| **B-20** 🟥 | **Frente COMERCIAL — lista de leads TDC lado CEDENTE** (venda de créditos). Estratégia lavrada em `docs/ESTRATEGIA-LEADS-TDC-CEDENTE-2026-06-28.md` (direção do MOU 2026-06-28). **C0 (local, desbloqueado):** engine cedente `pcpt_cedente`/`pcpt_doacao` (Art. 125/127). **C1/C2 (Drive):** universo ZEPEC/ZEPAM por lote + Atc/dono. **C3:** casamento cedente↔receptor (Art. 128). | C0: `engines/tdc/oodc.py` com `pcpt_cedente(atc,cabas,fi=1)` + `pcpt_doacao(atc,camax,fi)` citando Art. 125/127, `_autoteste` verde, e `scripts/gerar_leads_cedentes.py` provado em amostra. C1+: lista ranqueada por valor de crédito (PCpt×V) em tiers A/B/C, excluindo AUE/APPa (Art. 124 §2º). | C0 nenhum (local) · C1/C2 Drive/geo (perímetros ZEPEC/ZEPAM + cadastro — via B-9) |
