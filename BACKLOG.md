...
> - **AUD-A01** 🟦 `pcpt.py` deve LER `tabelas/fi-*.csv` (hoje hardcoda) + autoteste comparar engine×CSV. DoD: sabotar o CSV FALHA o gate.
> - **AUD-A10** 🟦 backfill `fonte.hash` (sha256) nos 63 .json + ligar o gate "hash confere" no `consolidar.py`. DoD: 0 `fonte.hash=null`.
> - **AUD-A11** 🟥→dono INGERIR o TDC normativo (Decreto 57.536/2016 + decretos ZEPEC) em `leis/` verbatim, OU manter a lacuna declarada (README/DISCLAIMER já declaram). DoD: `dominio tdc` no corpus > 1 lei.
> - **AUD-B08/B09** 🟦 completar schema 2.4 (campos vazios) + datar `vigencia.inicio` das 12 municipais `bruto` (depende do verbatim do Drive, B-4).
> - **AUD-C05/C06** ⬜ data da redação nova em chunk `compilado`; separar anexos do PDE (Art.174) em chunk `tipo=anexo`.
> - **AUD-A08** 🟦 gate de arrumação: check REAL contra o Drive (C2 contagem/C3 confronto via MCP, sob gate do dono) — hoje a prova é o log colado.
> - **AUD-B01** ⬜ fixture de domínio INDEPENDENTE (o `carimbar --check` é tautológico); revisão humana (`revisado_por_humano`) de ≥1 lei-âncora.
...
| **B-4** 🟦 | **Re-ingerir as 14 leis MUNICIPAIS em verbatim integral** (só resumo WebSearch hoje). | cada `.md` com `## Texto integral (verbatim)` + `.json confianca:alta` + fatiada/indexada; rodar `python3 scripts/promover_entrada.py <id>` quando o cru chegar a `_entrada/`. MANIFESTO: `indexado` sobe de **19** (vivo na auditoria 2026-07-05; era 13 quando o item nasceu). | cru NÃO está local — Drive (B-9) |
