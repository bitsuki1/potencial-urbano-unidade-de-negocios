...
| **B-20** 🟥 | **Frente COMERCIAL — ENRIQUECER as planilhas ZEPEC** (a lista oficial de cedentes). Direção MOU 2026-06-28: **AGNÓSTICO — sem valor, sem opinião, sem cruzar receptor.** Plano em `docs/ENRIQUECIMENTO-PLANILHAS-ZEPEC-2026-06-28.md` (camadas C-a..C-f, proveniência por campo, prior art MEGA_PLANILHA a auditar). _(Doc anterior `ESTRATEGIA-LEADS-TDC-CEDENTE` SUPERADO — valor/tiers/casamento rejeitados.)_ | **Local (desbloqueado):** schema do enriquecimento + normalizador C-a (SQL_MESTRE/ENDERECO_MESTRE) provado sobre amostra das colunas nativas ZEPEC. **Drive (B-9):** cargas C-c..C-f (IPTU/SISSEL/socios/geo). Cada campo carimba `fonte/ano/oficialidade/confianca`; derivado entra como alerta (D-03/D-04). | C-a local · C-b..C-f Drive (planilhas ZEPEC + cadastro/geo — via B-9). **4 decisões abertas do MOU no §7 do plano.** |
