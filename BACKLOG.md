...
| **B-22** 🟨 | **Sincronizar notas de confiança `.md` ↔ `.json` — ✅ FEITO (PU 15, 2026-07-01).** Os 4 `.md` (stf-tema-94, stf-tema-155, stj-resp-1202136, stj-resp-1645832) que diziam "confiança média" foram reconciliados para **ALTA** (coerente com `verificacao_verbatim` do `.json`), **preservando a nuance da antítese/vacina** de cada um. Tema 1350 mantém média legítima nos dois lados (recente, a confirmar) — não é dessinc. | 4 pares coerentes; gate verde (sem regressão). | — (resolvido) |
