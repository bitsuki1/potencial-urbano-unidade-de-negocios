...
- **`status_pipeline=bruto` é honesto:** ~26 itens seguem `confianca:baixa/media` (resumos não-verbatim) aguardando re-ingestão verbatim do Drive — ver `HANDOFF` §3 e `MANIFESTO.json` `alertas`. _(nota 2026-06-20: as 12 federais já são verbatim (8 com confiança média de extração); os 15 municipais-SP seguem como resumos não-verbatim.)_ Não declarar "pronto" o que é resumo.
