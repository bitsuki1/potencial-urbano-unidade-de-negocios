...
- [x] **ESTEIRA RAG construída e provada fim-a-fim — FEITO (2026-06-20).** `scripts/fatiar.py`→`indexar.py`→`consultar.py`→`evals/`: chunking por dispositivo (2.5), índice BM25 + metadados (2.6), consulta com **citação obrigatória** (1.7) e gate de cobertura. 6/6 evals ATIVOS verdes sobre a Lei 7.228/1968. `consolidar.yml` roda a cadeia + evals como gate. Determinístico, sem LLM (1.3/1.4). Doc: `scripts/README.md`. **Bloqueio p/ a fatia ser de PRODUTO (TDC):** falta verbatim TDC (egress .gov.br=403, Drive=lane exclusiva) — ver PROXIMA-INSTANCIA P2.
- [x] **Lei municipal 7.228/1968 — re-ingerida VERBATIM** (de `_entrada/misto/`), `confianca:alta`, `status_pipeline=indexado` (1ª lei a passar de `tagueado`).
- [x] **12 leis FEDERAIS — FEITO** (upload do MOU 2026-06-19; **promovidas a verbatim + fatiadas + indexadas em 2026-06-20** via `promover_entrada.py` — `status_pipeline=indexado`, 1.246 chunks no total). _Supera o item antigo "não estão no despejo" e o status `bruto`/"aguardando fatiamento"._
